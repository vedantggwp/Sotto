...
class CommandParser:
    ...
    COMMAND_PATTERNS = [
        ...
        (
            r"^open\s+(quote|parenthesis|bracket)$",
            "insert_punctuation",
            lambda m: {"char": {"quote": '"', "parenthesis": "(", "bracket": "["}[m.group(1)]},
        ),
        (
            r"^close\s+(quote|parenthesis|bracket)$",
            "insert_punctuation",
            lambda m: {"char": {"quote": '"', "parenthesis": ")", "bracket": "]"}[m.group(1)]},
        ),
    ]
    ...
    def parse(self, text: str) -> ParsedIntent:
        ...
        """
        if not text:
            return ParsedIntent(intent_type=IntentType.DICTATION, text="")
        ...
        return ParsedIntent(intent_type=IntentType.DICTATION, text=text)

    def add_custom_command(self, pattern: str, command_name: str, arg_extractor=None):
