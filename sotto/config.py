...
class HotkeyConfig(BaseModel):
    ...
    toggle_listening: str = Field(
        default="<cmd>+<shift>+l", description="Toggle always-listening mode"
    )
...
class SottoConfig(BaseModel):
    ...
    mode: Literal["push_to_talk", "always_listening"] = Field(
        default="push_to_talk", description="Voice input mode"
    )
    ...
    def save(self) -> None:
        ...
        with open(CONFIG_FILE, "w") as f:
            ...
            # Use safe_dump to avoid Python-specific YAML tags
            data = self.model_dump(exclude={"is_listening"})
            # Recursively ensure all strings are Python native strings, not PyObjC or specific types
            def clean_data(d):
                if isinstance(d, dict):
                    return {k: clean_data(v) for k, v in d.items()}
                elif isinstance(d, list):
                    return [clean_data(v) for v in d]
                elif hasattr(d, 'upper'): # Is string-like
                    return str(d)
                return d

            cleaned_data = clean_data(data)
            yaml.safe_dump(cleaned_data, f, default_flow_style=False)
